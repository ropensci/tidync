...
The other requirement we have is the ability to automate these task, and so far we have only interacted with the dimensionality information from the print out. For programming, there are functions `hyper_vars()` and `hyper_dims()` to pull out a table of variables or dimensions. The value of these functions is restricted to the *active* grid, and the name of the grid is available via `active()`. 
...
Future releases aim to make the handling of these less raw. 
