NetCDF is a very widely used system for storing and distributing
scientific array data. If you want to automate your own workflow around
a series of NetCDF source data then
[tidync](https://docs.ropensci.org/tidync/) provides all the flexibility
and power required, with as little pain as possible.

The [tidyverse](https://www.tidyverse.org/) has had an enormous impact
on the use of R with a strict approach to [*variables* and
*observations*](https://r4ds.had.co.nz/tidy-data.html). This insight can
be used for any form of data but when it comes to large data obviously
we can’t store everything in one table. A single table also does not
know that it represents *arrays with dimension*, and this inherent
property of the original data may well be lost. There is a tension
between the **tidyverse** and **scientific array data** that comes down
to this difference in data storage, and the intermediate forms used to
get between one form and another. I think this *tension* has been
exaggerated in unproductive ways.

### tidync

The tidync package provides a compromise position, allowing efficient
array slicing by dimension coordinate or index, producing native-array
*or* tidy-data-frame output as desired, delaying any data-reading until
after the output format is chosen. In particular tidync exists in order
to reduce the amount of plumbing code required to get to the data, and
allows an interactive way to convert between coordinate spaces and index
spaces.
...
A given variable won’t necessarily use all the available dimensions and
...
dimension* and call that an ***axis***, subtley different to the
...
NetCDF can be used to store *raster data*, and very commonly data is
provided as a global grid of scientific data, here a snapshot of global
ocean surface temperature generated by blending remote sensing, local
observations and physical model output.

There’s an example file `reduced.nc` in the stars package, derived from
the daily [OISSTV2
...
The dimensions label, name, length, min (`dmin`) and max (`dmax`) value
are seen in the Dimensions table and these values can never change, also
see flags `unlim` (an unlimited dimension?) and `coord_dim`.
...
Some grids have more than one variable, so they are nested in the grid
...
variable\`, dimensions and attributes by name or by internal index.
...
And we can find the internal metadata for each variable by expanding the
value.
...
In terms of *interpreting the meaning of stored metadata*, tidync shies
...
*degenerate dimensions*, i.e. the data is really 2D but we have a record
of a 4D space from which they are expressed as a slice. This can cause
problems as we would usually treat this data as a matrix in R, and so
the ncdf4 and RNetCDF package read functions have arguments that are
...
The print summary of the `oisst_data` object shows that it knows there
...
We have been bitten by thinking that this source data is 2D! So we just
