...
# file.copy("~/Git/tidync/inst/blog-01/tidync.md", "~/Git/roweb2/content/blog/2019-11-05-tidync.md", overwrite = TRUE)
...
There are various other packages for NetCDF in R, the main ones being [RNetCDF](https://CRAN.r-project.org/package=RNetCDF) and [ncdf4](https://CRAN.r-project.org/package=ncdf4). These are both *lower-level* tools than tidync - they are interfaces to the underlying NetCDF library, and tidync uses both to read information and data. The [raster](https://CRAN.r-project.org/package=raster), and [stars](https://CRAN.r-project.org/package=stars) packages provide quite different approaches and stars is more general than raster, but is similarly *higher-level* than tidync. 
...
In terms of *interpreting the meaning of stored metadata*, tidync shies away from doing this automatically. There are simply too many ways for automatic tools to get intentions wrong. So, used in combination the ncmeta and tidync packages provide the tools to program around the vagaries presented by NetCDF sources. If what you want is software that aims to interpret all this for you then check out [stars](https://CRAN.r-project.org/package=stars) the R package, and outside of R itself the various high-level applications capable of dealing with NetCDF: [xarray](http://xarray.pydata.org), [GDAL](https://gdal.org/), [ferret](https://ferret.pmel.noaa.gov/Ferret/), and [Panoply](https://www.giss.nasa.gov/tools/panoply/). 
...
This is exactly the data provided by `ncdf4::ncvar_get()` or `RNetCDF::var.get.nc()` but we can do it in a single line of code without finding the
...
The print summary has updated the `start` and `count` columns now to match our laboriously acquired versions above (they are slightly different because of the difference between `findInterval()` and our inequality expressions). 
...
One unfortunate issue here is that we cannot use the transforms directly, they *have* been updated by changing the value of the `selected` column from `TRUE` to `FALSE`, but then we have to be aware of using only the values that remain selected (not filtered out).  
...
For those that prefer old-school ggplot2 we can read our slice in directly as a tibble data frame. 
...
As a further example, now open a *time-series* NetCDF file, apply a spatial subset on the `lon` and `lat` dimensions, convert to tidy data frame and plot the `tos` variable spatially with faceted plots on the `time` dimension. 
...
As an alternative, we can choose the middle value of longitude (it lies at index = 90) and plot the `tos` variable as a function of latitude over time. We can easily re-orient our approach to this data set and it works as well with more complicated multi-dimensional sources as well. 
...
* Unknowns about what is supported on what platforms. This is surprisingly tricky and unstable, there are a lot of things that are possible on one operating system at a given time, but not on others. The situation changes fairly slowly but is
